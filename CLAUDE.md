...
- **Run formatting + static analysis before every commit.** Always run `just format-fix` (clang-format) first, then the static-analysis suite — at minimum `just lint` (format + shellcheck + launch-config checks), and `just cppcheck`, `just tidy`, and `just semgrep` for any change touching C/H sources. Fix or justify every finding before committing; these are the same checks CI runs.
