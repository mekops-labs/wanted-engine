...
| Drivers | `PlatformDriverTable` — the platform's additions to the launch-config driver names (`wifi` on NuttX, `wifi`/`ota` on ESP-IDF, none on Linux). `gpio` is a core driver instead: its tree and grant grammar are identical everywhere, and only the line behind it is per-platform, behind `PlatformGpio*`. A build may add a third table from a tree outside this repo; see [Out-of-tree drivers](#out-of-tree-drivers) |
