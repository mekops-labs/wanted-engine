...
- **Board watchdog** — complements the per-slot OTA revert path: that catches an unconfirmed image that never boots, this catches one that runs but wedges, with nothing else left to notice. Armed at 8 s when the wapp loop starts, and kicked every second by a thread one priority step above the supervisor. That priority is the point: a wapp runs at the loop's own priority and the supervisor one step above it, so a reconcile that computes for seconds preempts the loop entirely — measured at 4 s per tick and about 8 s on the first tick after boot on a Pico 2 Plus W. Kicking from the loop alone resets a working board. The kicker is not a free pass: it kicks only while the loop keeps calling `BoardWdtHeartbeat`, and stops 30 s after those stop, so a wedged engine still resets — verified on hardware, 37.6 s from the last heartbeat to the reset. The watchdog alone cannot catch a provisional boot whose supervisor comes up but never gets healthy, since it keeps kicking; a separate 45 s revert deadline (`OTA_REVERT_DEADLINE_S`, sized from a measured ~13 s boot-to-supervisor with headroom, matching the ESP-IDF backing's own deadline) reverts that case.
...
The seed list is configuration, not code: `CONFIG_WANTED_SEED_WAPPS` carries the space-separated `wapps/<name>` stems a board factory-seeds, and only an ESP-IDF build factory-seeds today — every other platform ignores it. Empty means a board seeds nothing beyond what its own `WANTED_EXTRA_SEEDS` names.
