...
- **Compile-time memory caps.** Per-instance limits live in `src/include/wanted-config.h`: `WASM_STACK_SIZE`, `WASM_HEAP_SIZE`, `WASM_MAX_MEMORY_PAGES`. The cap is enforced two ways — WAMR bounds `memory.grow` at runtime, and the engine refuses at load any image whose declared *initial* memory exceeds the cap (otherwise WAMR clamps the cap up to the module's initial, letting a large initial bypass the runtime bound). `just memcap` is the negative test that verifies the cap actually bounds a `memory.grow`.
