...
- The vendored backend does not reject a **low-order** public key. It decodes the key, then runs the cofactorless check `S·B − h·A == R`. A key that decodes to a point of small order — the all-zero 32 bytes is one such point, of order 4 — lets an attacker forge a valid-looking signature for it without the matching private key: pick `R` as the encoded identity point and `S = 0`, then find any message whose hash reduces to zero mod the point's order. A caller must never treat a decodable key as a trustworthy one; a key set must reject an all-zero or otherwise low-order value before it reaches `/dev/ed25519`.

What this layer guarantees: a wapp can verify a signature without ever holding the verifying key material in its own linear memory, and a build without a crypto backend cannot pretend to verify. What it does not: it does not provision keys, rotate them, or attest to the Deputy — that is the supervisor's job — and it does not validate that a public key is well-formed beyond its byte length; a degenerate key is the caller's responsibility to exclude.
...
- Console pipes **deliver or throttle**: a full ring short-writes, then holds the writer until the reader drains, and returns `EAGAIN` at the safety cap. An unread peer slows its producer and costs it no bytes. A wapp cannot DoS the engine by spamming stdout — the ring is bounded and the writer waits on it.
