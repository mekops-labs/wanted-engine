...
- **`platform/nuttx/`** — the NuttX port: the CI-gated simulator plus the RP2350 boards; cooperative stop interrupted by `SIGUSR2`; vendored `orlp/ed25519` verify; no TLS.
- **`platform/esp-idf/`** — the native ESP-IDF port for the whole ESP32 family (ESP32-S3 and the classic ESP32): FreeRTOS via the pthread wrapper, a flash LittleFS registry, PSRAM, A/B OTA on a part with the slots for it, mbedTLS sockets.
