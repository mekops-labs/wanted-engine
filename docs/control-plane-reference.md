...
| `mounts` | array | Up to 10 file/backend drivers, each bound at an arbitrary absolute `path` outside `/dev`, `/proc` and `/net`. The `platform` backend binds a host directory as a native WASI preopen (a bind mount; `options` set the host source and access mode — see below); other backends mount through the VFS router. |
