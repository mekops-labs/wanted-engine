...
| `pipe` | console slot | Live console: backs the slot with a named pipe in the shared store, so a peer wapp reads the stream at `/dev/pipe/<name>`. The pipe is auto-named `<wapp>.<slot>` (e.g. `app.out`) unless `options` pins `name=`. A full ring short-writes and then returns `EAGAIN`, so a slow reader throttles the writer and no bytes are dropped; `in` reads a peer's writes and treats a writer's absence as "nothing yet", not end-of-stream. | `name=feed` |
