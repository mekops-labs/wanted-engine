...
2. **Compiled-in default** — with no argument, the engine uses the launch config selected at build time (`WANTED_DEFAULT_CONFIG`, default `configs/example_config.json`), compiled into the binary. See [Platform Guide → selecting the compiled-in launch config](platform-guide.md#selecting-the-compiled-in-launch-config).
...
| `pipe` | console slot | Live console: backs the slot with a named pipe in the shared store, so a peer wapp reads the stream at `/dev/pipe/<name>`. The pipe is auto-named `<wapp>.<slot>` (e.g. `app.out`) unless `options` pins `name=`. A full ring short-writes, then holds the writer until the reader drains, and returns `EAGAIN` at the safety cap, so a slow reader throttles the writer and no bytes are dropped; `in` reads a peer's writes and treats a writer's absence as "nothing yet", not end-of-stream. | `name=feed` |
