...
int PlatformClockNanoSleep(plat_clk_id_t clk_id, plat_timestamp_t timeout,
    ...
    /* clock_nanosleep returns the error number directly and does not set
     * errno. Surface it so a stop signal's EINTR unwinds the host call back to
     * the interpreter, where the terminate flag is honoured. */
