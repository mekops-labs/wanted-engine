...
/* Serial-number sources, most hardware-tied first. The device tree carries one
 * on the ARM boards this target runs on; the DMI nodes cover x86, and the
 * kernel keeps them root-readable only, so an unprivileged engine falls through
 * them. /etc/machine-id is deliberately not a source: it identifies an OS
 * install rather than the silicon, and is regenerated by a reinstall. */
#define SERIAL_SOURCE_DT "/proc/device-tree/serial-number"
#define SERIAL_SOURCE_DMI_BOARD "/sys/class/dmi/id/board_serial"
#define SERIAL_SOURCE_DMI_UUID "/sys/class/dmi/id/product_uuid"

/* What firmware writes into a DMI field it has no value for. */
static int serialIsPlaceholder(const char *s) {
    static const char *const junk[] = {"None",
                                       "Default string",
...
/* Trimmed contents of `path` into `buf`; its length, or -ENOENT if unusable. */
static int serialFromFile(const char *path, char *buf, size_t bufLen) {
    FILE *f = fopen(path, "r");
    if (f == NULL)
        return -ENOENT;
    ...
    /* The device-tree property carries its terminating NUL, and any of these
     * sources may be newline- or space-padded. */
    while (n > 0 && (buf[n - 1] == '\n' || buf[n - 1] == '\r' ||
                     buf[n - 1] == ' ' || buf[n - 1] == '\0')) {
        buf[--n] = '\0';
    }
    if (n == 0 || serialIsPlaceholder(buf))
        return -ENOENT;
    return (int)n;
}
...
int PlatformSerialNumber(char *buf, size_t bufLen) {
    static const char *const sources[] = {
        SERIAL_SOURCE_DT, SERIAL_SOURCE_DMI_BOARD, SERIAL_SOURCE_DMI_UUID};
    ...
    for (size_t i = 0; i < sizeof(sources) / sizeof(sources[0]); i++) {
        int n = serialFromFile(sources[i], buf, bufLen);
        if (n > 0)
            return n;
    }
    buf[0] = '\0';
