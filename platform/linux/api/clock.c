...
static inline
int convert_clockid(plat_clk_id_t in) {
    switch (in) {
    case PLAT_CLOCKID_REALTIME:             return CLOCK_REALTIME;
    case PLAT_CLOCKID_MONOTONIC:            return CLOCK_MONOTONIC;
    //case PLAT_CLOCKID_PROCESS_CPUTIME_ID:   return CLOCK_PROCESS_CPUTIME_ID;
    //case PLAT_CLOCKID_THREAD_CPUTIME_ID:    return CLOCK_THREAD_CPUTIME_ID;
    default: return -1;
    }
}

static inline
plat_timestamp_t convert_timespec(const struct timespec *ts) {
    if (ts->tv_sec < 0)
        return 0;
...
static inline
struct timespec convert_timestamp(plat_timestamp_t t) {
    struct timespec ts;
...
{
    if (resolution == NULL) return -EINVAL;
