...
src_dirs := "src platform cmd include test"
...
# non-reproducible), and test/unity/ for the same reason.
#
# Three files carry data rather than code and are excluded by name:
# wanted-config.h and test_wasi.tar.c are generated byte arrays, and
# ctrl-action.json.h holds JSON inside STRINGIFY() that clang-format reflows.

# Reject any formatting drift (clang-format config in .clang-format).
lint-format:
    find {{src_dirs}} \( -name build -o -name 'build.*' -o -name managed_components -o -name .cache -o -name unity \) -prune -o \( -name '*.c' -o -name '*.h' \) ! -name wanted-config.h ! -name test_wasi.tar.c ! -name ctrl-action.json.h -print0 \
