...
function(SETUP_TARGET_FOR_COVERAGE _targetname _testrunner _outputname)
    ...
    add_custom_target(
        ...
        # ${_outputname}.info
        COMMAND ${LCOV_PATH} --directory . --capture --output-file
                ${_outputname}.info --rc lcov_branch_coverage=1
...
function(SETUP_TARGET_FOR_COVERAGE_COBERTURA _targetname _testrunner
    ...
    add_custom_target(
        ...
        # argument verbatim, so embedded quotes become literal and match
        # nothing. Search only the build tree (last arg) for .gcda — the report
        # root and filter stay at the source. Scanning the whole source root
        # makes gcovr trip over stray .gcda left in third_party/ by an
        # out-of-tree build (e.g. the NuttX sim) on a reused CI workspace.
        COMMAND
            ${GCOVR_PATH} --exclude-unreachable-branches --print-summary --txt
            -r ${CMAKE_SOURCE_DIR} -f ${CMAKE_SOURCE_DIR}/src/
            ${CMAKE_BINARY_DIR}
        COMMAND
            ${GCOVR_PATH} --xml-pretty --exclude-unreachable-branches -r
            ${CMAKE_SOURCE_DIR} -f ${CMAKE_SOURCE_DIR}/src/ -o
            ${_outputname}.xml ${CMAKE_BINARY_DIR}
        WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
